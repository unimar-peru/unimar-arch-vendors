# Guía de Contribución — unimar-arch-vendors
...
2. **Taxonomía heredada (S-18).** La estructura de este repositorio la rige la [Taxonomía de Repositorio Satélite](https://github.com/unimar-peru/unimar_arch/blob/main/reference/governance/standards/taxonomia-repositorio-satelite.md) — `kebab-case`, sin directorios sin scope. Se **referencia, no se copia**: no mantengas una versión local.
3. **Frontera de carpetas.** Este satélite es un paquete documental sin código ejecutable: la raíz de contenido canónico es `estandar/` (portal en el README raíz). Ver D-002 en [DECISIONS.md](./DECISIONS.md). No mezclar dominios.
...
3. **Abre la propuesta como PR en `unimar_arch`**, contra la fuente del estándar (`.harness/`), nunca contra una copia local. Si el cambio es una decisión técnica, lleva su ADR.
   ...
   ```bash
   gh pr create --repo unimar-peru/unimar_arch --title "..." --body "Origen: unimar-arch-vendors, gap G-NNN"
   ```
...
| :--- | :--- | :--- |
| `estandar/` | Contenido canónico del estándar para proveedores (documentación) | Taxonomía documental (S-18 Extend, D-002) |
...
| :--- | :--- | :--- | :--- |
| 1.0.0 | 2026-07-24 | unimar-arch-vendors | Materialización desde la plantilla del plugin `unimar-core` |
